...
class cartComponent extends Component {
	...
	getCartList() {
		// axios.get(`${REQUEST_PATH.BaseUrl}/api/v1/customer/CartList/${this.state.customerDetails._id}`)
		// 	.then(response => {
		// 		var sum = 0
		...
		// 		console.log(error);
		// 	});
		getRequest(REQUEST_PATH.cartList(this.state.customerDetails._id))
    	.then(response => {
				let sum = 0
				response.data.object.object[0].productDetails.forEach(function (data) {
					sum = sum + data.price
	...
	removeFromCart(productData) {
		...
		// axios.put(`${REQUEST_PATH.BaseUrl}/api/v1/customer/Cart/REMOVE`, {
		...
		putRequest(REQUEST_PATH.removeCart, payload)
			.then(response => {
				toast.success("Product Removed to Cart");
				this.getCartList();
			})
													...
													<tr>
														<td>
															<Link to={`${process.env.PUBLIC_URL}/left-sidebar/product/${item._id}`}>
																<img src={(item.productImage != "") ? REQUEST_PATH.BaseUrl + '/' + item.productImage : this.state.defaultImage} alt="" />
